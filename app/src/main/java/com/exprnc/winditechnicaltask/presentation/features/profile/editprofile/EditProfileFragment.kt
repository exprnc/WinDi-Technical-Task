...
import com.exprnc.winditechnicaltask.presentation.features.auth.registration.RegView
...
class EditProfileFragment : BaseFragment() {
    ...
    @Inject
    lateinit var assistedFactory: EditProfileViewModel.Factory
    private val viewModel: EditProfileViewModel by viewModels {
        EditProfileViewModel.provideFactory(
            EditProfileArgs.fetchBundleData(requireArguments()),
            assistedFactory
        )
    }
    ...
    override fun Content() {
        ...
        when (viewState) {
            is EditProfileState.DefaultState -> EditProfileView(viewState, viewModel::obtainIntent)
        }
        ...
        viewModel.viewEvent.flowWithLifecycle(viewLifecycleOwner.lifecycle).onEach { event ->
            when (event) {
                ...
                is ViewEvent.PopBackStack -> {
                    ...
                    fm.setFragmentResult(EditProfileScreen.TAG, event.bundle)
                }
            }
